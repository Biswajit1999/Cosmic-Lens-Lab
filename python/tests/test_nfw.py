...
def test_nfw_shape_is_finite_from_centre_to_outer_halo():
    assert nfw_potential(0.0, 0.19, 0.62) == 0.0
    for x in (1e-8, 1e-6, 1e-4, 1e-3, 0.01, 0.1, 0.9, 1.0, 1.1, 5.0):
...
def test_nfw_radial_derivative_matches_deflection():
    kappa_s = 0.19
    radius_scale = 0.62
    for radius in (1e-4, 5e-4, 0.001, 0.006, 0.03, 0.11, 0.37, 0.61, 0.93, 2.2):
        h = max(1e-9, radius * 1e-5)
